...
# The two validation (val) scripts in this repo are for performing solution validaiton
...
# from b_... import ...concave # getting orientation of cutting planes on concave biseciton


class ValintsCalcClass:
    def __init__(self, S1, S2, SVal, Fiducial):
        # Class attributes
        self.S1 = S1
        self.S2 = S2
    ...
    def FittingPoly(self):
        ...
        self.SVal.coeffx = np.polyfit(self.SVal.ztup, self.SVal.xtup, 3) # Get the coefficients for a parametertised 3rd order polynomial as function Z
        self.SVal.coeffy  = np.polyfit(self.SVal.ztup, self.SVal.ytup, 3) # Get the coefficients for a parametertised 3rd order polynomial as function Z
    ...
    def geometricTangCalc(self, dmatrix):
        ...
        for i in range(len(dmatrix)):
            # SplinesNorm, Tang_to_splines = geometricEqs.tangentLine(i, self.S1, self.S2) # making tangent vector using average deravitive
            # avgTang2spline = geometricEqs.tangentLine(i, self.S1, self.S2) # making tangent vector using average deravitive
            avgTang2spline, ptXZ1, ptXZ2, ptYZ1, ptYZ2 = geometricEqs.tangentLine(i, self.S1, self.S2) # making tangent vector using average deravitive
            norm_2spline, VecA2B, midpoint = geometricEqs.normalSpline(i, avgTang2spline)

            '''Calculating plane using normal vector and a on the curve'''
    ...
    def find_intersection_points(self, a, b, c, d):
        """
        Finds the intersection point between a 3rd order polynomial and a 2D plane.

        Args:
            coeff_x: Coefficients of x(z) in the polynomial.
            coeff_y: Coefficients of y(z) in the polynomial.
            a, b, c, d: Coefficients of the plane equation ax + by + cz + d = 0.

        Returns:
            Tuple (x, y, z) representing the intersection point.
        """
        def equations(vars):
            x, y, z = vars
            return [
                np.polyval(self.SVal.coeffx, z) - x,
                np.polyval(self.SVal.coeffy, z) - y,
                a * x + b * y + c * z + d
            ]

        initial_guess = (0, 0, 0)  # Initial guess for (x, y, z)
